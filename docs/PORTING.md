...
| `qjs.c`, `repl.js` | the `ljs` program; the REPL's bytecode generated by ljsc | `tests/ljs.lucb`, `tests/repl.js`, `tests/repl.lucb` |
| `qjsc.c` | the `ljsc` program (writes Luce files) | `tests/ljsc/` |
...
- `tests/tools.py` (run by `./test.sh`) tests ljs's options, exit statuses and REPL
  ...
  (`examples/`, with its fib native module) and a module with imports (`tests/tools/`)
  with ljsc (executable, `-e` with a native module, `-c`) and
  compares the programs' output with ljs's. With `QJSC=path/to/qjsc` it also checks that
...
- test262: `python3 tests/test262.py` clones tc39/test262 at the commit QuickJS pins
  ...
  reported as CRASH by the split run. `python3 tests/test262.py --suite es5` runs the old
  ES5 suite as `make test2o` does (tests/test262o.conf; the es5-tests branch of test262 is
  cloned outside the repository): `2/11261 errors, 464 excluded`, the same two
  (15.2.3.6-4-538-3 and -7) as C QuickJS, whose test262o_errors.txt is empty.

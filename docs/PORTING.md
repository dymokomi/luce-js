...
- `tests/tools.py` (run by `./test.sh`) tests ljs's options, exit statuses and REPL
  ...
  -m repl.js` after a change of the bytecode format), and compiles QuickJS's examples and
  a module with imports with ljsc (executable, `-e` with a native module, `-c`) and
